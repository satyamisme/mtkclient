...
class Stage2(metaclass=LogBase):
    ...
    def memwrite(self, start, data):
        ...
        while bytestowrite>0:
            ...
            self.cdc.usbwrite(pack(">I", 0xf00dd00d))
            self.cdc.usbwrite(pack(">I", 0x5001))
            self.cdc.usbwrite(pack(">I", addr+pos))
            self.cdc.usbwrite(pack(">I", size))
            self.cdc.usbwrite(data)
            bytestowrite-=len(data)
            pos+=len(data)
...
def main():
    ...
    if st2.connect():
        if args.rpmb:
            st2.rpmb(start,length,filename=os.path.join("logs","rpmb"))
        elif args.preloader:
            st2.preloader(start,length,filename=os.path.join("logs","preloader"))
        elif args.memread:
